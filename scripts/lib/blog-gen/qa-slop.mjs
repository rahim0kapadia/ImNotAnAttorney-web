...
function buildA1Prompt(mdxContent) {
...
function evaluateA1Results(results) {
...
function parseA1Response(responseText) {
  ...
  return parsed.map((item, idx) => {
    if (typeof item !== "object" || item === null) {
      return { check: `UNKNOWN_${idx}`, result: "FAIL", reason: "Invalid response item" };
    }
    return {
      check: typeof item.check === "string" ? item.check : `UNKNOWN_${idx}`,
      result: ["PASS", "FAIL", "NEEDS_WORK"].includes(item.result) ? item.result : "FAIL",
...
/**
 * Run the A1 slop audit on MDX content. Pure function.
 * @param {string} mdxContent                                - Full MDX source (frontmatter + body)
 * @param {object} _supabase                                 - Reserved (preserved from web version)
 * @param {{callClaude: Function, jobId?: string}} ctx       - Injected gateway + optional jobId
 */
export async function runSlopAudit(mdxContent, _supabase, { callClaude, jobId }) {
  const { text } = await callClaude({
    jobType: "blog_qa_slop",
    systemPrompt: "",
    userPrompt: buildA1Prompt(mdxContent),
    maxTokens: 2048,
    jobId,
    purpose: "blog_qa_slop",
  });

  const checkResults = parseA1Response(text);
