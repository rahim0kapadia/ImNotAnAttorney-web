// csv-bulk-checked: https://www.courtlistener.com/api/rest/v4/search/?type=o&court=haw&q=%22Office+of+Disciplinary+Counsel%22 — CL search returns 362 Hawaii Supreme Court Office of Disciplinary Counsel opinions (verified 2026-04-27); HI ODC (dbhawaii.org) publishes per-decision PDFs but lacks structured per-attorney listings. CL is the structured bulk surface.
// Template: scripts/ingest/scrape-ctbar-discipline.mjs
// Pattern: cl-bulk-data-defensive #18 (COPY FROM STDIN via bulkCopyRows)
//
// Hawaii attorney-discipline scraper.
//
// Source discovery (probed 2026-04-27):
//   Primary: CourtListener search API — court=haw (Hawaii Supreme Court)
//     https://www.courtlistener.com/api/rest/v4/search/?type=o&court=haw&q=%22Office+of+Disciplinary+Counsel%22
//     Returns 362 HI Supreme Court attorney-discipline opinions. Caption pattern
//     "Office of Disciplinary Counsel v. <Name>" (forward-only) plus
//     "Disciplinary Board of the Hawai'i Supreme Court v. <Name>". Per-sanction
//     queries anchor each result's snippet on sanction language.
//
//   Fallback / verification: HI ODC — https://www.dbhawaii.org/ — Hawaii's
//     Office of Disciplinary Counsel and Disciplinary Board publish discipline
//     orders. CL captures these as published opinions.
//
// HI SC docket format: SCAD-NN-NNNNNNN, SCPR-NN-NNNNNNN, SCPW-NN-NNNNNNN.
// bar_number = "HISC:<docketNumber>" — deterministic, idempotent.
//
// Caption: "Office of Disciplinary Counsel v. <Name>", "Disciplinary Board of
// the Hawai'i Supreme Court v. <Name>", "In re: <Name>", "In re <Name>".
// REJECT: reverse "<Name> v. Office of Disciplinary Counsel" (challenges).
// REJECT: "<Name> v. Disciplinary Board" (challenges).
//
// Discipline label mapping (HI → internal enum): standard set.
//
// Polite scraping: 800-1600 ms randomized delay between CL page requests.
// UA identifies INAA per project convention.
//
// Usage:
//   node scripts/ingest/scrape-hibar-discipline.mjs                         # dry-run
//   node scripts/ingest/scrape-hibar-discipline.mjs --apply                 # write to DB
//   node scripts/ingest/scrape-hibar-discipline.mjs --help
...
async function fetchJson(url, attempt = 1) {
  ...
  if (resp.status === 429 || resp.status === 503 || resp.status === 502) {
    ...
    console.error(`[hi] HTTP ${resp.status} attempt ${attempt}, backing off ${ms}ms`);
    await sleep(ms);
...
//
// HI captions:
//   ACCEPT: "Office of Disciplinary Counsel v. <Name>"
//           "Disciplinary Board of the Hawai'i Supreme Court v. <Name>"
//           "In re: <Name>" / "In re <Name>"
//   REJECT: "<Name> v. Office of Disciplinary Counsel" (challenges)
//           "<Name> v. Disciplinary Board ..." (challenges)
//           "Disciplinary Board v. <Court>" reflexive
export function parseCaseName(caseName) {
  ...
  if (/\bv\.\s+(?:the\s+)?(?:office\s+of\s+disciplinary\s+counsel|disciplinary\s+board)/i.test(raw)) {
    return { fullName: null };
  }

  let name = raw
    .replace(/^Office\s+of\s+Disciplinary\s+Counsel\s+v\.?\s*/i, '')
    .replace(/^Disciplinary\s+Board\s+of\s+the\s+Hawai[ʻ']?i\s+Supreme\s+Court\s+v\.?\s*/i, '')
    .replace(/^Disciplinary\s+Board\s+v\.?\s*/i, '')
    .replace(/^In\s+re:?\s*/i, '')
    .replace(/^Petition\s+of\s+/i, '')
    .replace(/\.\s*Opinion\s+by\s+.*$/i, '')
    .replace(/\s*\[ada\]\s*$/i, '')
    .replace(/,?\s*Concurring\s+in\s+Part.*$/i, '')
    .replace(/\s*,\s*$/, '')
    .trim();

  if (name === raw.replace(/^Petition\s+of\s+/i, '').trim()) {
    // Caption did not match any allowed prefix
    if (!/^In\s+re/i.test(raw)) return { fullName: null };
  }
  // Reject if name still contains a court/board/office phrase
  if (/\b(disciplinary\s+(board|counsel)|supreme\s+court|hawaii\s+supreme)\b/i.test(name)) {
    return { fullName: null };
  }
...
// HI dockets: SCAD-NN-NNNNNNN, SCPR-NN-NNNNNNN, SCPW-NN-NNNNNNN, SCBE-NN-NNNNNNN
export function isHiDocket(docket) {
  if (typeof docket !== 'string') return false;
  const cleaned = docket.replace(/<\/?mark>/g, '').trim();
  return /^(SCAD|SCPR|SCPW|SCBE|SCWC)-\d{2}-\d{6,7}$/i.test(cleaned);
}

export function normalizeHiDocket(docket) {
...
async function discoverViaCl() {
  ...
  for (const [qFragment, sanctionType] of SANCTION_QUERIES) {
    const baseQ = `("Office of Disciplinary Counsel" OR "Disciplinary Board") AND (${qFragment})`;
    let url =
      CL_SEARCH_BASE +
...
async function load(records) {
  ...
  try {
    ...
    await client.query(`DROP TABLE IF EXISTS public._stg_attorneys_hi`);
    await client.query(`DROP TABLE IF EXISTS public._stg_discipline_hi`);
    await client.query(`
      CREATE UNLOGGED TABLE public._stg_attorneys_hi (
      ...
      CREATE UNLOGGED TABLE public._stg_discipline_hi (
    ...
    const upsertA = await client.query(`
      ...
      SELECT jurisdiction, bar_number, full_name, first_name, last_name,
        admission_date, current_status, city, source_url, NOW() FROM _stg_attorneys_hi
    ...
    await client.query(`DROP TABLE IF EXISTS public._stg_attorneys_hi, public._stg_discipline_hi`);
  } finally {
    await cleanup();
  }
}
...
export function buildRecordFromClResult(r, assertedSanctionType) {
  ...
  if (!isHiDocket(docket)) return null;
  ...
  const normDocket = normalizeHiDocket(docket);
  const barNumber = `HISC:${normDocket}`;
...
async function main() {
  console.error(`[hibar] start — apply=${OPTS.apply} startDate=${OPTS.startDate}` +
  ...
  console.error(`[hibar] collected ${records.length} discipline rows`);
  for (const r of records.slice(0, 3)) {
    console.error(`  · [${r.bar_number}] ${r.full_name} — ${r.discipline_type} (order_date=${r.order_date || '?'}) — ${r.source_url}`);
  }
  if (!OPTS.apply) {
    console.error('[hibar] dry-run — pass --apply to write to DB');
    return;
  }
  if (records.length === 0) {
    console.error('[hibar] no records — nothing to load');
    return;
  }
  await load(records);
  console.error('[hibar] done');
}
