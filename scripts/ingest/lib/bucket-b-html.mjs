 ...
 * @property {Function} [fetchHtml]   Optional override for the HTTP GET. Same signature as
 *                                    fetchWithRetry(url, {timeoutMs, userAgent}) -> string.
 *                                    States blocked by Cloudflare TLS-fingerprint detection
 *                                    on undici (e.g. capitol.hawaii.gov) supply a curl-based
 *                                    shim. When undefined, the default fetchWithRetry is used.
 */
...
export async function fetchSection(desc, config) {
  ...
  const fetcher = config.fetchHtml || fetchWithRetry;
  const html = await fetcher(desc.sectionUrl, {
...
export async function ingestBucketBState(config, opts = {}) {
  ...
  const tocFetcher = config.fetchHtml || fetchWithRetry;
  const tocHtml = await tocFetcher(config.chapterListUrl, {
