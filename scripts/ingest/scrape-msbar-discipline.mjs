// csv-bulk-checked: none-exists — CourtListener search API used for per-sanction MS Supreme Court bar discipline filtering. CL has 491 'Mississippi Bar' opinions and 359 disbarment opinions on court=miss (verified 2026-04-27). The 50GB opinions bulk file would require streaming + re-filtering for ~225 'Mississippi Bar v.' BD-suffix opinions; per-sanction search API is the established bulk-surface pattern (see scrape-okbar-discipline.mjs, PR #185 merged 2026-04-27). msbar.org publishes NO public list of disciplined attorneys (verified 2026-04-27 — only phone-call disclosure per /ethics-discipline/disciplinary-process/disciplinary-history/), so MS Supreme Court opinions are the authoritative bulk surface.
// Template: scripts/ingest/scrape-okbar-discipline.mjs
// Pattern: cl-bulk-data-defensive #18 (COPY FROM STDIN via bulkCopyRows)
//
// Mississippi Supreme Court bar discipline scraper.
//
// Source discovery (probed 2026-04-27):
//   Primary: CourtListener search API — court=miss + per-sanction queries
//     https://www.courtlistener.com/api/rest/v4/search/?type=o&court=miss&q=...
//
//   Mississippi Bar publishes NO public list of disciplined attorneys —
//   www.msbar.org/ethics-discipline/disciplinary-process/disciplinary-history/
//   only states the public can phone the OGC for current status. The MS
//   Supreme Court opinions are the authoritative public source for completed
//   discipline matters (Bar Discipline + Bar Reinstatement docket suffixes).
//
// MS docket format:
//   YYYY-BD-NNNNN-SCT  → Bar Discipline (originating discipline matters — INCLUDE)
//   YYYY-BR-NNNNN-SCT  → Bar Reinstatement (later petition — SKIP for now;
//                        avoids double-counting the same attorney as a new event)
//
// Caption shapes:
//   "The Mississippi Bar v. <Name>"           → originating discipline (INCLUDE)
//   "<Name> v. The Mississippi Bar"           → reinstatement appeal (SKIP)
//   "In Re The Petition of <Name>..."         → reinstatement (SKIP — BR docket)
//   "In Re: <Name>"                            → may be discipline; require BD docket
//
// Filter: docket suffix must be `-BD-` to count as an originating discipline event.
// Reinstatement events (`-BR-`) are skipped to avoid duplicating the discipline
// for the same attorney across multiple opinions.
//
// Discipline label mapping (MS SC → internal enum):
...
//
// bar_number = "MS:<docketNumber>" — deterministic, idempotent. MS Bar bar
// numbers (4-5 digits) appear in opinion text but are not in search metadata;
// docket-based identity is the reliable join key (also matches PR #181 MO/WI/LA
// + PR #184 NC/AL/SC + PR #185 OK/OR/CT synthetic-key convention).
//
// Polite scraping: 800-1600 ms randomized delay between CL page requests.
//
// Usage:
//   node scripts/ingest/scrape-msbar-discipline.mjs                         # dry-run
//   node scripts/ingest/scrape-msbar-discipline.mjs --apply                 # write to DB
//   node scripts/ingest/scrape-msbar-discipline.mjs --start-date 2010-01-01 --apply
//   node scripts/ingest/scrape-msbar-discipline.mjs --help
...
const CL_SEARCH_BASE =
  ...
  '?type=o&court=miss&format=json&order_by=dateFiled+desc&highlight=on';
...
export const ALLOWED_DISCIPLINE_TYPES = new Set([
  'disbarment', 'suspension', 'interim_suspension',
...
async function fetchJson(url, attempt = 1) {
  ...
  if (resp.status === 429 || resp.status === 503 || resp.status === 502) {
    ...
    console.error(`[ms] HTTP ${resp.status} attempt ${attempt}, backing off ${ms}ms`);
    await sleep(ms);
...
// MS captions:
//   "The Mississippi Bar v. Charlie A. Carr"            → disciplined = "Charlie A. Carr"
//   "The Mississippi Bar v. Guy N. Rogers, Jr."         → disciplined = "Guy N. Rogers, Jr."
//   "Christina Huffman Bennett v. The Mississippi Bar"  → reversed (reinstatement appeal); SKIP
//   "Joe Gregory Stewart v. The Mississippi Bar"        → reversed; SKIP
//   "In Re: <Name>"                                      → only with BD docket
//
...
export function parseCaseName(rawCaption) {
  ...
  // Reversed captions: Name v. The Mississippi Bar → reinstatement; skip
  if (/v\.\s+(the\s+)?mississippi\s+bar\b/i.test(caption)) {
    return { fullName: null };
  }

  // "The Mississippi Bar v. <Name>"
  const fwd = caption.match(/^the\s+mississippi\s+bar\s+v\.?\s+(.+?)\s*$/i);
  if (fwd) {
    const name = fwd[1].trim();
    if (name.length < 2 || name.length > 150) return { fullName: null };
    if (/\b(supreme court|disciplinary tribunal|complaint tribunal)\b/i.test(name)) {
      return { fullName: null };
    }
    return { fullName: name };
  }

  // "In Re: <Name>" — only valid when docket is BD; caller checks docket separately.
  const inre = caption.match(/^in\s+re[:\s]\s*(.+?)\s*$/i);
  if (inre) {
    const name = inre[1].replace(/^the\s+petition\s+of\s+/i, '').trim();
    if (name.length < 2 || name.length > 150) return { fullName: null };
    return { fullName: name };
  }
...
// MS docket: "YYYY-BD-NNNNN-SCT" or "YYYY-BR-NNNNN-SCT"
export function isMsBdDocket(docket) {
  if (typeof docket !== 'string') return false;
  return /^\d{4}-BD-\d{4,5}-SCT$/i.test(docket.trim());
}

export function isMsBrDocket(docket) {
  if (typeof docket !== 'string') return false;
  return /^\d{4}-BR-\d{4,5}-SCT$/i.test(docket.trim());
}
...
async function discoverViaCl() {
  ...
  for (const [qFragment, sanctionType] of SANCTION_QUERIES) {
    const baseQ = `"Mississippi Bar" AND (${qFragment})`;
    let url =
      CL_SEARCH_BASE +
...
async function load(records) {
  ...
  try {
    ...
    await client.query(`DROP TABLE IF EXISTS public._stg_attorneys_ms`);
    await client.query(`DROP TABLE IF EXISTS public._stg_discipline_ms`);
    await client.query(`
      CREATE UNLOGGED TABLE public._stg_attorneys_ms (
        jurisdiction    char(2),
        bar_number      text,
        full_name       text,
        first_name      text,
      ...
      CREATE UNLOGGED TABLE public._stg_discipline_ms (
        jurisdiction      char(2),
        bar_number        text,
        full_name         text,
        order_date        date,
    ...
    await client.query(`DROP TABLE IF EXISTS public._stg_attorneys_ms, public._stg_discipline_ms`);
  } finally {
    await cleanup();
  }
}
...
export function buildRecordFromClResult(r, assertedSanctionType) {
  ...
  // Only count BD (Bar Discipline) — skip BR (reinstatement appeals)
  if (!isMsBdDocket(docket)) return null;
...
async function main() {
  console.error(
    `[msbar] start — apply=${OPTS.apply} startDate=${OPTS.startDate}` +
  ...
  console.error(`[msbar] collected ${records.length} discipline rows`);

  const preview = records.slice(0, 3);
  console.error('[msbar] first 3 rows:');
  ...
  if (!OPTS.apply) {
    console.error('[msbar] dry-run — pass --apply to write to DB');
    return;
  }

  if (records.length === 0) {
    console.error('[msbar] no records — nothing to load');
    return;
  }

  await load(records);
  console.error('[msbar] done');
}
