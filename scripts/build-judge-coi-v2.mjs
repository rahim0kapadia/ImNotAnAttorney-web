// Template: scripts/build-judge-coi-actionable.mjs
// Expert: lukas-fittl
// Pattern: cl-bulk-data-defensive #17 (Albe) + two-phase filter
// csv-bulk-checked: N/A
// work-mem: Medium tier verified (256MB)
//
// Two-phase build: (1) fast broad INSERT (threshold 0.85, LIMIT 10 per jc-pair),
// (2) DELETE rows where judge is not on the case. Both phases are bounded; the
// INSERT is bounded by LIMIT, the DELETE uses the tmp_case_judges_norm index.
...
async function main() {
  ...
  try {
    ...
    console.log('STEP A: broad INSERT (threshold 0.85, LIMIT 10 per judge-company)');
    ...
    const rA = await c.query(`
      ...
      SELECT
        ...
        CASE WHEN matched.sim >= 0.95 THEN 'exact'
             WHEN matched.sim >= 0.85 THEN 'fuzzy'
      ...
      WHERE length(jc.company_norm) >= 6
        AND matched.sim >= 0.85
      ON CONFLICT (judge_canonical_id, case_canonical_id, investment_id) DO NOTHING
    `);
    console.log(`  INSERT: ${rA.rowCount} rows in ${((Date.now() - tA)/1000).toFixed(1)}s`);

    console.log('STEP B: post-filter DELETE — keep only rows where judge actually on case');
    const tB = Date.now();
    const rB = await c.query(`
      DELETE FROM judge_conflict_of_interest coi
      WHERE NOT EXISTS (
        SELECT 1 FROM tmp_case_judges_norm tj
        JOIN entities_judges ej ON upper(ej.name_last) = tj.judge_last_upper
        WHERE tj.case_canonical_id = coi.case_canonical_id
          AND ej.canonical_id = coi.judge_canonical_id
      )
    `);
    console.log(`  DELETE: ${rB.rowCount} rows (removed non-actionable) in ${((Date.now() - tB)/1000).toFixed(1)}s`);
