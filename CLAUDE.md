...
**⚠ DEPLOY SCOPE — read before any app-code change:** As of the **2026-07-06 self-host cutover**, `imnotanattorney.com` is served by the **netcup box** (`root@152.53.194.188`), which auto-deploys from **`ImNotAnAttorney` (monorepo) `master`, `apps/web/`** — NOT this repo, and NOT Vercel (retired; rollback-only). Pushing app-code fixes to `ImNotAnAttorney-web/master` MERGES BUT DOES NOT SHIP. For any change that must reach prod (`/src/`, `/supabase/functions/`, `/scripts/` runtime artifacts, `/content/`, `/public/`), work in `C:\Users\email\projects\ImNotAnAttorney\apps\web\` and push monorepo master — the box ships it in ≤10 min (see monorepo CLAUDE.md → "Self-Hosted Deployment"). This repo is legacy (blog drafts, Twitter queue, docs); its `generate-worker.mjs` still runs the box's Case Decoder backup cron from a frozen copy at `/opt/inaa-worker`. History: 2026-04-28 Vercel moved to monorepo (`gotcha-vercel-project-cutover-silent-abandon.md`); 2026-07-06 Vercel → box (gbrain `inaa-cutover-complete-2026-07-06`).
