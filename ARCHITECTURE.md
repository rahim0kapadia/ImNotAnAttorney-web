...
| **Per-Tier Generation Mode** | **PLANNED / IN-FLIGHT** — dispatcher that routes case-decoder generation to `api` / `mechanical` / `hybrid` / `session` modes via a `tier_generation_config` table. 3 routes (`/api/admin/session-report/[caseId]`, `/api/cron/notify-session-handoff`, `/api/reports/hybrid/[caseId]`) built on `feat/tier-ladder-deferred-closure` (commit `131e3199`); 6 artifacts still missing (`mode-config.ts`, `render-case-decoder.ts`, `generate-session-handoff.mjs`, `tier_generation_config` table + migration, `cases.session_generation_payload`/`generator_mode`). Not yet on master. | [`docs/plans/2026-04-23-per-tier-generation-mode-orphan-resume.md`](docs/plans/2026-04-23-per-tier-generation-mode-orphan-resume.md) |
