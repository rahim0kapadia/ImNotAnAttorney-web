...
export function TestimonialSection({ testimonials, variant }: TestimonialSectionProps) {
      ...
      <div className="flex flex-col gap-6 md:flex-row">
        {testimonials.map((t, i) => (
          <FadeInUp key={i} delay={i * 0.15} className="flex-1">
            <div className="rounded-xl border-l-4 border-amber-500/50 bg-zinc-900/50 p-6">
  ...
  return (
    <StaggerContainer className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {testimonials.map((t, i) => (
        <StaggerItem key={i}>
          <div className="rounded-xl border-l-4 border-amber-500/50 bg-zinc-900/50 p-6 h-full">
            <p className="text-sm leading-relaxed text-zinc-300 italic">
              &ldquo;{t.quote}&rdquo;
            </p>
            <div className="mt-4">
