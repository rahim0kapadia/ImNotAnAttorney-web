/**
 * State-Specific Domestic Violence Defense Page
 * /domestic-violence-defense/[state]
 *
...
export async function generateMetadata({
  ...
  const title = `${data.name} Domestic Violence Defense, Penalties & What to Ask`;
  const description = `${data.name} domestic violence: ${data.statuteNumber} (${data.offenseClass}). Penalties up to ${data.penaltyMax}. Enhancements, defense questions, and what to do next.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/domestic-violence-defense/${state}`,
    },
    openGraph: {
      title,
      description,
      url: `${SITE_URL}/domestic-violence-defense/${state}`,
      type: "website",
    },
  };
}

export default async function StateDvPage({ params }: PageProps) {
  const { state } = await params;
  const data = getStateDvLawsData(state);
  ...
  const playbook = (TIER_CORE as Record<string, typeof TIER_CORE[keyof typeof TIER_CORE] | undefined>)["domestic-violence"] ?? TIER_CORE["dui-first-offense"];
          ...
          __html: JSON.stringify({
            ...
            itemListElement: [
              { "@type": "ListItem", position: 1, name: "Home", item: SITE_URL },
              {
                ...
                item: `${SITE_URL}/domestic-violence-defense`,
              },
          ...
          <div className="rounded-lg border border-red-500/30 bg-red-500/5 p-5">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-400">
                  A conviction for misdemeanor domestic violence in {data.name} carries a
                  <strong className="text-white"> federal</strong> firearms-possession ban under
                  18 U.S.C. &sect; 922(g)(9). This consequence is collateral to {data.name} state
                  penalties and applies regardless of the state sentence imposed.
                </p>
              </div>
            </div>
          </div>
      ...
      <FadeInUp>
        <section className="mt-16 text-center">
          <h2 className="text-2xl font-bold text-white">
            Is your {data.name} domestic violence case defense on track?
          </h2>
          <p className="mt-3 text-sm text-zinc-400">
            The free Defense Score checks 10 critical defense behaviors specific to
            domestic violence cases. Takes 2 minutes. Instant results.
          </p>
          <Link
            href="/score?charge=domestic-violence"
      ...
      <section className="mt-12">
        <div className="rounded-lg border border-zinc-500 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-400">
            <strong>Important:</strong> This page provides legal INFORMATION about {data.name} domestic violence
            law as of the date of publication. Laws change frequently. This is not legal
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: `What is the maximum sentence for domestic violence in ${data.name}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `Under ${data.statuteNumber} (${data.offenseClass}), ${data.name} domestic violence carries a maximum sentence of ${data.penaltyMax} and a maximum fine of ${data.fineMax}.`,
                },
              },
              {
                "@type": "Question",
                name: `Does ${data.name} have a mandatory minimum for domestic violence?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: data.mandatoryMinimum
                    ? `Yes. ${data.name} imposes a mandatory minimum of ${data.mandatoryMinimum} for domestic violence. This applies unless an exception or diversion program is available.`
                    : `${data.name} does not impose a blanket mandatory minimum for simple domestic violence. Specific charges or enhancements may carry their own minimums.`,
                },
              },
              {
                "@type": "Question",
                name: `What factors enhance a domestic violence charge in ${data.name}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text:
                    data.enhancements.length > 0
                      ? `In ${data.name}, domestic violence penalties can be enhanced by: ${data.enhancements.join("; ")}.`
