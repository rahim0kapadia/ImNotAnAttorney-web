/**
 * State-Specific Assault Defense Page
 * /assault-defense/[state]
 *
...
export async function generateMetadata({
  ...
  const title = `${data.name} Assault Defense, Penalties & What to Ask`;
  const description = `${data.name} assault: ${data.statuteNumber} (${data.offenseClass}). Penalties up to ${data.penaltyMax}. Enhancements, defense questions, and what to do next.`;

  return {
    title,
    description,
    alternates: {
      canonical: `${SITE_URL}/assault-defense/${state}`,
    },
    openGraph: {
      title,
      description,
      url: `${SITE_URL}/assault-defense/${state}`,
      type: "website",
    },
  };
}

export default async function StateAssaultPage({ params }: PageProps) {
  const { state } = await params;
  const data = getStateAssaultLawsData(state);
  ...
  const playbook = (TIER_CORE as Record<string, typeof TIER_CORE[keyof typeof TIER_CORE] | undefined>)["assault"] ?? TIER_CORE["dui-first-offense"];
      ...
      <FadeInUp>
        <section className="mt-16 text-center">
          <h2 className="text-2xl font-bold text-white">
            Is your {data.name} assault case defense on track?
          </h2>
          <p className="mt-3 text-sm text-zinc-400">
            The free Defense Score checks 10 critical defense behaviors specific to
            assault cases. Takes 2 minutes. Instant results.
          </p>
          <Link
            href="/score?charge=assault"
      ...
      <section className="mt-12">
        <div className="rounded-lg border border-zinc-500 bg-zinc-900/50 p-4">
          <p className="text-xs text-zinc-400">
            <strong>Important:</strong> This page provides legal INFORMATION about {data.name} assault
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: `What is the maximum sentence for assault in ${data.name}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: `Under ${data.statuteNumber} (${data.offenseClass}), ${data.name} assault carries a maximum sentence of ${data.penaltyMax} and a maximum fine of ${data.fineMax}.`,
                },
              },
              {
                "@type": "Question",
                name: `Does ${data.name} have a mandatory minimum for assault?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text: data.mandatoryMinimum
                    ? `Yes. ${data.name} imposes a mandatory minimum of ${data.mandatoryMinimum} for assault. This applies unless an exception or diversion program is available.`
                    : `${data.name} does not impose a blanket mandatory minimum for simple assault. Specific charges or enhancements may carry their own minimums.`,
                },
              },
              {
                "@type": "Question",
                name: `What factors enhance a assault charge in ${data.name}?`,
                acceptedAnswer: {
                  "@type": "Answer",
                  text:
                    data.enhancements.length > 0
                      ? `In ${data.name}, assault penalties can be enhanced by: ${data.enhancements.join("; ")}.`
