...
const FAQS = [
  ...
  },
  {
    ...
    a: "Commissions are tracked in real time. We process payouts monthly via Venmo, Zelle, or check — your choice. You can see your running total and referral history anytime in your partner dashboard.",
  },
  {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/partners/apply", {
        ...
        body: JSON.stringify({ name, company, email, phone, region, message, source: "bondsman" }),
      });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
        ...
        <div className="max-w-5xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="w-14 h-14 bg-amber-500 text-black rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">1</div>
              <h3 className="text-lg font-bold mb-2">Get Your Code</h3>
              <p className="text-zinc-400">Apply below. Once approved, you get a unique promo code and a simple one-pager to hand defendants.</p>
            </div>
            <div className="text-center">
              <div className="w-14 h-14 bg-amber-500 text-black rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">2</div>
              <h3 className="text-lg font-bold mb-2">Share It</h3>
              <p className="text-zinc-400">Hand the code to defendants when they bond out. They enter it at checkout for 10% off any service.</p>
            </div>
            <div className="text-center">
              <div className="w-14 h-14 bg-amber-500 text-black rounded-full flex items-center justify-center text-2xl font-bold mx-auto mb-4">3</div>
              <h3 className="text-lg font-bold mb-2">Earn Commission</h3>
              <p className="text-zinc-400">Every time someone uses your code, you earn 10% commission. Tracked automatically. Paid monthly.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-5xl mx-auto px-6 py-16">
        ...
        <p className="text-center text-zinc-500 text-sm mt-4">
          One X-Ray referral = {COMMISSION_TABLE.find(r => r.tier.includes("X-Ray"))?.commission}. Five referrals a month = serious passive income.
        </p>
      </section>
        ...
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-left">
            <div className="bg-zinc-800 rounded-xl p-6">
              ...
              <p className="text-zinc-400">Most purchases happen within 7 days of arrest — the exact window when you&apos;re handing them your business card.</p>
            </div>
            <div className="bg-zinc-800 rounded-xl p-6">
              <p className="font-bold text-amber-400 mb-2">Zero Explanation</p>
              <p className="text-zinc-400">Defendants already know they need help. You just hand them a card with a code. The website does the selling.</p>
            </div>
            <div className="bg-zinc-800 rounded-xl p-6">
              <p className="font-bold text-amber-400 mb-2">Helps Your Client</p>
              <p className="text-zinc-400">Better-informed defendants make better decisions. This is genuinely useful — not a gimmick.</p>
            </div>
            <div className="bg-zinc-800 rounded-xl p-6">
              <p className="font-bold text-amber-400 mb-2">Passive Income</p>
              <p className="text-zinc-400">No selling, no follow-up, no tracking. Hand out codes, check your dashboard, collect payouts.</p>
            </div>
          </div>
        </div>
      </section>
          ...
          {FAQS.map((faq, i) => (
            <div key={i} className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
              ...
              >
                ...
                <span className="text-zinc-400 text-xl ml-4">{openFaq === i ? "\u2212" : "+"}</span>
              </button>
              {openFaq === i && <div className="px-6 pb-4 text-zinc-400">{faq.a}</div>}
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-2xl mx-auto px-6">
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {error && (
                <div className="bg-red-900/50 border border-red-700 text-red-300 px-4 py-2 rounded-lg">{error}</div>
              )}
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-zinc-400 mb-1">Your Name *</label>
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
                <div>
                  <label className="block text-sm text-zinc-400 mb-1">Company / Agency</label>
                  <input type="text" value={company} onChange={(e) => setCompany(e.target.value)} className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
                <div>
                  <label className="block text-sm text-zinc-400 mb-1">Email *</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
                <div>
                  <label className="block text-sm text-zinc-400 mb-1">Phone</label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm text-zinc-400 mb-1">Region / Service Area</label>
                  <input type="text" value={region} onChange={(e) => setRegion(e.target.value)} placeholder="e.g., Maricopa County, AZ" className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm text-zinc-400 mb-1">Anything else we should know?</label>
                  <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={3} className="w-full px-4 py-3 bg-zinc-800 rounded-lg border border-zinc-700 text-white" />
                </div>
              </div>
              <button type="submit" disabled={submitting} className="w-full py-4 bg-amber-500 text-black font-bold rounded-xl text-lg hover:bg-amber-400 disabled:opacity-50 transition-colors">
