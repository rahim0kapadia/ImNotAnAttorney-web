---
platform: youtube
type: short
project: inna
status: pending
created: 2026-03-13
agent: cc-youtube-engine
title: "Your Right to See ALL the Evidence Against You"
target_length: 57s
---

# YouTube Short #10: Your Right to See ALL the Evidence Against You

**Source topic:** discovery-rights-drug-cases, how-to-read-your-discovery
**Duration:** 57 seconds
**Format:** talking head with legal document visuals and text overlays

## Title (for upload)
You Have a Constitutional Right to See ALL the Evidence

## Description
Many defendants never see their own discovery. Their attorney "handles it." That's a problem. You have a constitutional right to see every piece of evidence being used against you.

This is legal information, not legal advice. We are not attorneys.

imnotanattorney.com

#discovery #bradyvmaryland #defendantrights #criminaldefense #evidence #knowyourrights

## Script
[0:00-0:04] HOOK:
[Direct to camera, incredulous tone]
"The legal system has a file on you. Police reports. Witness statements. Lab results. Body camera footage. Phone records. Everything they've gathered to put you in prison. Have you seen it? Most defendants haven't. We help you build one on them."

[0:04-0:14] THE RIGHT:
[TEXT ON SCREEN: "Brady v. Maryland: Your constitutional right"]
"Under Brady v. Maryland, a Supreme Court ruling, the prosecution is required to share ALL evidence with the defense. Including evidence that helps you. This isn't optional. It's a constitutional right. And Brady violations, prosecutors hiding exculpatory evidence, are one of the most common reasons cases get overturned on appeal."

[0:14-0:26] THE PROBLEM:
[TEXT ON SCREEN: "Most defendants never read their discovery."]
"Here's the problem. Most defendants never see their own discovery. Their attorney 'handles it.' Some attorneys actually tell clients not to worry about the paperwork. Don't accept that. You lived the events. You know what happened. You will spot things in those documents that a busy attorney might miss."

[0:26-0:37] WHAT TO LOOK FOR:
[TEXT ON SCREEN: "4 things to look for"]
"When you get your discovery, look for four things. One: contradictions between different officers' reports. Two: timeline gaps that don't add up. Three: evidence that should exist but is missing, like body camera footage. Four: anything that contradicts the prosecution's version of what happened."

[0:37-0:48] THE ASK:
[TEXT ON SCREEN: "3 questions for your attorney"]
"Ask your attorney three things. Have you received all discovery from the prosecution? Can I have a complete copy? Is there anything outstanding that hasn't been turned over? If they push back on giving you a copy, that's a red flag about how your case is being handled. Put the request in writing."

[0:48-0:57] CTA:
[TEXT ON SCREEN: "imnotanattorney.com"]
"The evidence is your case. Don't let someone else decide what you need to see. Know What They Know. Full discovery guide, link in bio."

## On-Screen Text
- [0:00] "Have YOU seen your file?"
- [0:04] "Brady v. Maryland: constitutional right"
- [0:14] "Most defendants never read their discovery."
- [0:26] "4 things to look for"
- [0:37] "3 questions for your attorney"
- [0:48] "imnotanattorney.com"

## Thumbnail Concept
A thick stack of legal documents/manila folder with "DISCOVERY" written on the tab. A magnifying glass hovers over it revealing hidden text that says "YOUR RIGHTS." Bold text: "HAVE YOU SEEN THIS?" White on dark background. Communicates hidden information instantly.
