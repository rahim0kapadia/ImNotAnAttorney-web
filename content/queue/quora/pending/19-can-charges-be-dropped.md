---
platform: quora
type: answer
project: inna
status: pending
created: 2026-03-11
agent: community-engine
title: "Quora Answer: Can Charges Be Dropped"
blog_source: can-criminal-charges-be-dropped
suggested_time: "Weekday 10:00 AM EST"
---

## Quora: INNA-Q19 | Can my criminal charges be dropped? How does that actually happen?
**Topics**: Charges Dropped, Criminal Dismissal, Prosecutorial Discretion, Criminal Defense, Defendant Rights
**SEO**: "can criminal charges be dropped" / "how to get charges dropped" / "charges dismissed criminal case"

Yes, criminal charges can be dropped. But not the way most people think.

Charges don't get dropped because you're a good person, because it was your first offense, or because you said sorry. They get dropped for specific, technical, legal reasons.

**Who has the power:**

The prosecutor -- not the judge, not the police, not the victim -- has almost exclusive power to drop charges. This is called prosecutorial discretion. They can decline to file, drop charges after filing (nolle prosequi), reduce charges, or offer diversion programs.

The judge can dismiss only for legal reasons: speedy trial violation, successful motion to dismiss, defective indictment, constitutional violation.

NOT the victim. "The victim doesn't want to press charges" is a TV myth. The state brings criminal charges. The victim can ask the prosecutor to drop them, but the prosecutor decides.

**How charges actually get dropped:**

Insufficient evidence. The prosecution reviews their case and realizes they can't prove it beyond a reasonable doubt. This happens when your attorney identifies every weakness and communicates those weaknesses to the prosecutor.

Constitutional violations. Illegal search, unlawful stop, Miranda violation, coerced confession. Evidence obtained through these violations gets suppressed. Without key evidence, the prosecution may drop rather than go to trial.

Witness problems. Key witness becomes unavailable, refuses to cooperate, or has devastating credibility issues (prior convictions, cooperating for their own deal, statements that contradict physical evidence).

Diversion programs. Many jurisdictions offer these for first-time offenders. Complete a program -- community service, counseling, treatment -- and charges are dismissed.

Pre-trial intervention. You agree to conditions (no new arrests, community service, restitution) and in exchange, charges drop after a compliance period.

**What does NOT get charges dropped:**

"It was my first offense" -- might affect your sentence, but doesn't automatically mean charges drop. "The victim doesn't want to press charges" -- the prosecutor decides. "I said I was sorry" -- apologies aren't a defense and can actually be used as evidence of guilt.

**What you can do:** Make sure your attorney is actively filing motions, investigating witnesses, communicating weaknesses to the prosecutor, exploring diversion options, and building your case for every possible defense. Charges don't drop themselves. Someone has to push.

**Image direction**: Flowchart showing the paths to charge dismissal | **Republish to**: Medium if 10K+ views
