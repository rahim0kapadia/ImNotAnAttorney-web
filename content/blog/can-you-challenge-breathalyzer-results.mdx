...
Every measurement instrument has a margin of error. Breathalyzers are no exception. Depending on the model and the conditions, published error margins sit somewhere between plus or minus 0.005 and plus or minus 0.02 BAC.
...
<!-- SOCIAL_SPINE
HOOK: You blew a number. Everyone treats it like a confession.
THREAD:
The breathalyzer does not measure your blood — it measures your breath and guesses.
The conversion ratio is a population average, not your ratio.
A missed calibration cycle means nobody can prove the machine was right.
Mouth alcohol from a single belch can inflate results by multiples, not decimals.
The machine cannot interview you about your medical history.
A test performed next to a running police radio is not a lab-validated test.
A reading of 0.08 with a 0.02 error margin could actually be 0.06.
The printout does not include error bars — the defense has to raise them.
The officer's training on the machine is not flawless execution at 2 AM in the rain.
Every measurement instrument has a margin of error, including this one.
SCREENSHOT: The breathalyzer assumes you are average — and if you are not, the number is wrong before it leaves the device.
SHARE_FRAME: If you blew a number and think the case is over, read this first.
GROUP_ANSWER: Less accurate than the courtroom treats them. Published error margins run 0.005 to 0.02 BAC under lab conditions. Field conditions are worse — calibration drift, temperature swings, radio interference, and operator shortcuts all push variance higher. A reading of 0.08 may represent a real BAC anywhere from 0.06 to 0.10.
-->
