...
> **MediaProcessor integration (2026-04-09):** Uploaded files are processed by the engine repo MediaProcessor (Dyson architecture). The upload route accepts PDF, DOCX, XLSX (added 2026-04-09), images, audio, and video. MIME validation includes magic byte checks with sub-format verification for OOXML files. The engine TIER_MEDIA_CAPS config determines which file types each tier can actually process. Files outside the tier capability are accepted but flagged for operator review.
