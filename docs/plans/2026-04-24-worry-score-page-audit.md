...
**SC-P2 (observation coverage + stress-state segmentation):** Audit JSON from SC-C3.1 contains ≥40 charge-specific rows. Per-charge-type row count ≥ `max_per_charge - 1`. Every one of the 40 tuples {10 charge types} × {no-attorney, has-attorney} × {early-window, mid-window} appears at least once as a distinct row. `stress_bands` column non-empty for every row (WARN-10). PASS = row-count + per-charge-balance + tuple-presence + stress-column-populated all true.
